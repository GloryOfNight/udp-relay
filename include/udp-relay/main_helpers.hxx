...
namespace ur
{
	// helper object for handling command-line arguments
	struct cl_var_ref
	{
		template <typename T>
		constexpr cl_var_ref(std::string_view name, T& value, std::string_view noteHelp)
			: m_name{name}
			, m_noteHelp{noteHelp}
			, m_value{&value}
			, m_type{typeid(T)}
	...
	// helper object for handling environment variables
	using env_var_ref = cl_var_ref;
...
{
	static_assert(std::is_same<typename T::value_type, cl_var_ref>::value, "T must be an array of val_ref");

	cl_var_ref const* prev_arg = nullptr;
	for (int i = 0; i < argc; ++i)
	{
		const std::string_view arg = argv[i];

		// find if argument listed in args
		const auto found_arg = std::find_if(std::begin(argList), std::end(argList), [&arg](const cl_var_ref& val)
...
void ur::parseEnvp(const T& envList, char* envp[])
{
	...
	for (int i = 0; envp[i] != NULL; ++i)
	{
		...
		if (found_env != std::end(envList))
		{
			...
			{
				std::println("Failed parse environment variable: {0}. Type not supported: {1}", found_env->m_name, found_env->m_type.name());
			}
		}
	}
}